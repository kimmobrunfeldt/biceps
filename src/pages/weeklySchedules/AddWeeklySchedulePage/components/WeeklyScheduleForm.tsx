...
const WeeklyScheduleFormSchema = ProductResolvedBeforeSavingSchema.superRefine(
  (values, ctx) => {
...
export type WeeklyScheduleFormFields = z.infer<typeof WeeklyScheduleFormSchema>

type Props = {
  initialData?: WeeklyScheduleFormFields
  onSubmit: (data: WeeklyScheduleFormFields) => void
...
export function ProductForm({
  ...
  } = useForm<WeeklyScheduleFormFields>({
    defaultValues: {
  ...
  const onSubmit = useCallback(
    async (data: WeeklyScheduleFormFields) => {
  ...
  const gramFields = keys.map((key) => ({
    key,
    label: getLabel(key),
  })) satisfies Array<{ key: keyof WeeklyScheduleFormFields; label: string }>
